...
const Destinations = () => {
  ...
  const locationName = destinationParams.get("location");
  const [locations, setLocations] = useState([]);
  const tourLocationRef = useRef(null);
  const sortSortRef = useRef(null);
  const [destinations] = useDestinations(filterValue, locationName);
  ...
  useEffect(() => {
    const handleOutsideClick = (e) => {
      if (
        tourLocationRef.current &&
        !tourLocationRef.current.contains(e.target)
      ) {
          ...
          <div className="flex items-center lg:gap-3 relative border bg-white p-1 rounded-lg">
            ...
            >
              ...
              <p className="text-sm text-gray-500 ">
                {searchDestination
                  ? searchDestination
                  : "Select your sectination"}
              </p>
            </div>

            {/* destination location list */}
            {showDestinationDropown && (
              <div
                ref={tourLocationRef}
                className="w-full lg:w-96  min-h-96 bg-white  absolute z-10 border-2 border-gray-500 rounded-lg  top-[70px]  p-2 "
              >
                <div className="flex items-center ">
                  <SearchIcon />
                  <input
                    className="w-full h-8 outline-none text-sm pl-2"
                ...
                <div className="w-full h-80 overflow-auto mt-1">
                  {filterLocation.map(({ id, title }) => (
                    <p
                      ...
                      key={id}
                      className="text-sm font-medium text-gray-500 hover:bg-[#E4E9F1] p-2 mt-1 rounded-lg cursor-pointer"
                    >
                      {title}
                    </p>
                  ))}
          ...
          <div className=" flex items-center gap-2  mt-2">
            <div className="lg:hidden">
              <button
                ...
                className="flex items-center bg-white p-2 rounded-lg font-semibold"
              >
                Filter <DownIcon />
              </button>

              {showFilter && (
                <div className="w-full  absolute bottom-0 bg-white shadow z-10 left-0  p-4">
            ...
            {destinations.length !== 0 ? (
              <>
                {destinations?.map((destination) => (
                  <Card key={destination._id} please={destination} />
                ))}
              </>
            ) : (
              <div className="flex items-center justify-center w-full bg-red-50 ">
                <p className="text-sm ">
                  {locationName} destination is not found
                </p>
              </div>
            )}
          </div>
